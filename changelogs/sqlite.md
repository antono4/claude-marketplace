...
### Fixed
- Constrained the `mcp` dependency to `>=1.9.4,<2` in the PEP 723 header of `scripts/mcp_sqlite_server.py` (and in `pyproject.toml`/`README.md`) — mcp 2.0.0 removed `mcp.server.fastmcp`, which the server imports, so any fresh `uv run --script` resolution crashed at startup with `ModuleNotFoundError: No module named 'mcp.server.fastmcp'`
